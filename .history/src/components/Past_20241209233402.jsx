...
function Past() {
  useEffect(() => {
    // Split the paragraph text into spans
    const paragraph = document.querySelector("#past-paragraph");
    if (paragraph) {
      const text = paragraph.textContent;
      const splittedText = text.split("");
      const clutter = splittedText.map((char, index) => `<span key=${index}>${char}</span>`).join("");
      paragraph.innerHTML = clutter;
    }

    // GSAP animation for paragraph spans
    gsap.to("#past-paragraph span", {
      color: "black", // Customize as needed
      stagger: 0.02,
      scrollTrigger: {
        trigger: "#past-paragraph",
        scroller: "#main", // Adjust if using a custom scroller
        start: "top 90%",
        end: "top 30%",
        scrub: 1,
        pin: false,
    ...
    <div className='h-[88vh] font-[font1] flex px-16 w-full'>
      <div className='relative w-1/2'>
        ...
        <div className='relative pl-16 py-2'>
          <h1 className='text-[7.3vw] -tracking-[5px] text-zinc-300 font-semibold'>
            OUR <span className='text-[#0D7C66] text-[7.5vw] pl-4 tracking-tight'>PAST</span>
          </h1>
          <p
            id="past-paragraph"
            className='mt-4 w-[85%] font-[font6] text-zinc-400 text-[2.3vh] font-[100] leading-7'
          >
