  ...
  useEffect(() => {
    ...
    cards.forEach((card, index) => {
      gsap.fromTo(card, { opacity: 0, x: 0 }, { // Start with opacity 0 and translate in x direction
        opacity: 1,
        x: 0, 
        delay: index * 0.8, // Add a delay based on the index of the card
        scrollTrigger: {
          trigger: card,
          left: "top 80%", // When the top of the card hits 80% of the viewport height
          right: "top 30%", // When the top of the card hits 30% of the viewport height
          toggleActions: "play none none reverse", // Play on enter, reverse on leave
          // Ensure each card is visible one by one
          onEnter: () => {
            gsap.to(card, { opacity: 1, delay: index * 0.2 }); // Add delay to the visibility
          },
          onLeaveBack: () => {
