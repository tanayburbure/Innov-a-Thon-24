...
const Schedule = () => {
  ...
  useEffect(() => {
    ...
    cards.forEach((card) => {
      gsap.fromTo(card, { opacity: 0, x: 2 }, { // Start with opacity 0 and translate in x direction
        opacity: 1,
        x: 2, 
        scrollTrigger: {
          trigger: card,
          ...
          onEnter: () => {
            gsap.to(card, { opacity: 1 }); // Make the card visible
          },
          onLeaveBack: () => {
            gsap.to(card, { opacity: 0 }); // Hide the card when scrolling back
          },
        },
      });
    });
