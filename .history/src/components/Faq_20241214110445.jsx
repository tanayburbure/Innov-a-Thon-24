...
function Faq() {
  ...
  return (
    ...
    <div className="h-[90vh] w-full lg:h-[100vh]">
      ...
      <div className="text-zinc-400 font-[font6] p-14 gap-8 mt-8 font-light lg:flex">
        <div className=" pt-[0.2px] text-[3vw] lg:text-[1.7vw]">
          {questions.slice(0, 6).map((question, index) => (
            <div key={index} className="mb-6 ">
              <h2
                onClick={() => toggleAnswer(index)}
                className="cursor-pointer hover:text-[#0D7C66] transition-all duration-300"
              >
                <span className="text-[4vw]">{showAnswer[index] ? '-' : '+'}</span> {question}
              </h2>
              {showAnswer[index] && (
                <p className="px-4 text-[2.5vw] lg:text-[1.5vw] text-zinc-300 mt-2">{answers[index]}</p>
              )}
            </div>
          ))}
        </div>
        <div className=" text-[3vw] lg:text-[1.7vw]"> 
          {questions.slice(6, 12).map((question, index) => (
            <div key={index + 6} className="mb-6 ">
              <h2
                onClick={() => toggleAnswer(index + 6)}
              ...
              {showAnswer[index + 6] && (
                <p className="px-4 text-[2.5vw] text-zinc-300 mt-2">{answers[index + 6]}</p>
              )}
            </div>
          ))}
        </div>
