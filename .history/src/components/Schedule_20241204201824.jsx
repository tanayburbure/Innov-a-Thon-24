...
const Schedule = () => {
  ...
  useEffect(() => {
    ...
    cards.forEach((card, index) => {
      gsap.fromTo(card, { opacity: 0, y: 20 }, { // Start with opacity 0 and translate in y direction
        opacity: 1,
        y: 0, 
        scrollTrigger: {
          trigger: card,
          start: `top ${80 + index * 10}%`, // Adjust start position for each card
          end: `top ${30 + index * 10}%`, // Adjust end position for each card
      ...
      <div className="flex w-[270vw] h-screen" ref={scrollContainerRef}>
        <div className="w-[200vw] h-screen flex items-center">
          ...
          <div className="absolute top-[40vh] left-[42vw] transform -translate-x-1/2 flex flex-col items-center">
            <div className="flex space-x-8 ml-[100vw] mt-[7vh] gap-[4vw] w-[170vw] ">
              {[
                ...
                <div className="text-white text-center" key={index}>
                  ...
                  <div className={`card w-[20vw] h-[22vh] bg-gray-800 rounded-lg shadow-lg mt-2 opacity-0 translate-x-4 transition-opacity duration-500 ease-in-out ${index % 2 === 0 ? 'translate-x-[-20px]' : 'translate-x-[20px]'}`}>
