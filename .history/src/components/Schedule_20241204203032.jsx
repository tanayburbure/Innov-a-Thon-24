  ...
  useEffect(() => {
    ...
    cards.forEach((card, index) => {
      gsap.fromTo(card, { opacity: 0, x: 0 }, { // Start with opacity 0 and translate in x direction
        opacity: 1,
        x: 0, 
        delay: index * 0.6, // Add a delay based on the index of the card
        scrollTrigger: {
          trigger: card,
          top: "top 80%", // When the top of the card hits 80% of the viewport height
          bottom: "top 30%", // Play on enter, reverse on leave
          // Ensure each card is visible one by one
          onEnter: () => {
            gsap.to(card, { opacity: 1, delay: index * 0.6 }); // Add delay to the visibility
          },
          onLeaveBack: () => {
            gsap.to(card, { opacity: 0, delay: index * 0.6 }); // Add delay when scrolling back up
          },
        },
      });
    });
