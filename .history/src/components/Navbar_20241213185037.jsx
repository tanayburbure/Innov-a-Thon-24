...
const Navbar = () => {
  ...
  return (
    ...
    //   `}</style>
    // </div>
    <div className="flex w-[70vw] justify-between font-[font1] pt-1 pr-3">
      {/* Logo */}
      ...
      {/* Navigation Links */}
      <div className="relative left-[60vw] text-[#B3C8CF] tracking-wide z-50 lg:text-[2.4vh] text-[2.2vh] flex justify-end mt-4 lg:mt-5 space-x-8">
        <div className="">
          <button className="text-gray-400 border-none font-bold md:hidden" onClick={() => setMenuOpen(!menuOpen)}>
            {menuOpen ? <FaTimes /> : <FaBars />}
          </button>
          <ul className={`flex-col md:flex-row md:flex ${menuOpen ? 'flex' : 'hidden'} text-gray-400 font-bold`}>
            {[
              { name: "About", scrollPercentage: 185 },
              { name: "Prizes", scrollPercentage: 100 },
              { name: "Sponsors", scrollPercentage: 266 },
              { name: "FAQ", scrollPercentage: 680 },
            ].map((item, index) => (
              <li
                key={index}
              ...
              >
                {item.name}
                <span className="absolute bottom-0 left-1 w-[90%] lg:w-[30%] h-[2px] bg-[#006A67] scale-x-0 origin-left transition-transform duration-400 group-hover:scale-x-100"></span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          {/* Register Button */}
        <span className="relative top-4 -right-8 whitespace-nowrap lg:left-[20vw] lg:">
          <button className="border-none bg-[#41B3A2] overflow-hidden pb-[0.5vh] text-black font-semibold rounded-full z-[99999] fixed">
            <span className="inline-block animate-marquee font-[font4] text-[1.6vh] lg:text-[2.2vh] lg:px-2 lg:pt-[2.5px]">
              Register Here
            </span>
          </button>
        </span>
